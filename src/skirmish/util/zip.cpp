...
public:
    explicit in_zip_file_stream(util::in_stream& zip, bool& open, uint64_t uncompressed_size, uint32_t crc32)
        : uncompressed_{zip}
        , open_(open)
        , pos_(0)
...
private:
    util::in_deflate_stream uncompressed_;
    bool&                   open_;
    size_t                  pos_;
    size_t                  size_;
    uint32_t                crc32_;
    ...
    util::array_view<uint8_t> refill_in_zip_file_stream() {
        ...
        if (pos_ + buf.size() >= size_ && uncompressed_.crc32() != crc32_) {
            assert(false);
            return set_failed(std::make_error_code(std::errc::io_error));
        }
        return buf;
...
public:
    ...
    std::unique_ptr<util::in_stream> get_file_stream(const std::string& filename) {
        ...
        assert(ch.compression_method == compression_methods::deflated);
        open_file_ = true;
        return std::make_unique<in_zip_file_stream>(zip_, open_file_, ch.uncompressed_size, ch.crc32);
    }
